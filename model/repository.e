...
feature -- Abstraction Function
	...
	model: FUN[KEY, TUPLE[d1: DATA1; d2: DATA2]]
		...
		local
			loctuple: TUPLE[d1: DATA1; d2: DATA2]

		p: PAIR[KEY, TUPLE[d1:DATA1; d2: DATA2]]
		funmod: FUN[KEY, TUPLE[DATA1, DATA2]]

		do
			-- TODO: Implement the abstraction function.
			create Result.make_empty
--			create loctuple.default_create
--			create loctuple.default_create
			create funmod.make_empty

			across keys as i loop
				create loctuple.default_create
				loctuple.d1 := data_items_1[keys.index_of (i.item, 1)]
				loctuple.put (data_items_2.at (i.item), 2)
				create p.make (i.item, loctuple)
				funmod.extend (p)
...
--			across keys as i loop funmod.extend end




--			across keys is i loop result.extend(i) end
--			across keys as i loop result.extend(create {PAIR[, TUPLE[DATA1, DATA2]])   end
...
--				across keys is i all keys[i] ~ model. end
				TRUE
	...
	make
		...
		do
			...
			empty_repository: -- TODO:
				keys.is_empty
				data_items_1.is_empty
				data_items_2.is_empty
				model.is_empty
		end
	...
	check_in (d1: DATA1; d2: DATA2; k: KEY)
		...
		do
			...
			repository_count_incremented: -- TODO:
				old keys.count ~ keys.count - 1
				old data_items_1.count ~ data_items_1.count -1
				old data_items_2.count ~ data_items_2.count -1
...
--				across keys is t all old keys.at (t) ~ keys.at (t)  end
--				across data_items_1 is t all old data_items_1[t] ~ data_items_1[t] end
	...
	matching_keys (d1: DATA1; d2: DATA2): ITERABLE[KEY]
		...
		do
			...
			result_contains_correct_keys_only: -- TODO:
				...
				-- If you can't get this postcondition to work, simply put TRUE here.
--				across result as r all
--					across model is m some
--							m.first ~ r
--						end
--					end
					TRUE
